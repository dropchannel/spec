...
| [0039](./ADR-0039.md) | Branch naming conventions | Adopt `{type}/{description}` name format, where type is one of:`feat`,`fix`,`refactor`,`docs`,`chore`,`adr`. | `{type}/{scope}/{description}` (scope is redundant for isolated repos), `{repo}/{type}/{description}` (Not relevant for isolated repos) |
